...
# turn into vkernel), we will call a sub make in that other dir, and
...
# Use VKERNELINCLUDE when you must reference the include/ directory.
...
# ===========================================================================
# Build targets only - this includes vkernel, arch specific targets, clean
...
# Defaults to vkernel, but the arch makefile usually adds further targets
all: vkernel
...
vkernel-libs	:= $(patsubst %,%/lib.a, $(libs-y))
...
$(sort $(vkernel-all)): $(vkernel-dirs) ;

# Handle descending into subdirectories listed in $(vkernel-dirs)
...
define filechk_version.h
	(echo \#define VKERNEL_VERSION_CODE $(shell                         \
	expr $(VERSION) \* 65536 + 0$(PATCHLEVEL) \* 256 + 0$(SUBLEVEL)); \
	echo '#define VKERNEL_VERSION(a,b,c) (((a) << 16) + ((b) << 8) + (c))';)
endef
...
CLEAN_DIRS  +=
CLEAN_FILES +=	vkernel vkernel.img
...
clean-dirs      := $(addprefix _clean_, . $(vkernel-dirs))
...
help:
	...
	@echo  '* vkernel		  - Build the application'
